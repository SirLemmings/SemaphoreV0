...
class IdentityProcessor:
    ...
    def cache_alias_mint(self, alias: Alias, pubkey: Pubkey) -> None:
        ...
        if alias.value == b"\xff\xff\xff\xff": #RULE 3: alias cannot be 0xffffffff
            return
    ...
    def step(self) -> None:
        ...
        for alias, pubkey in cached_alias_mints:
            if alias in seen_aliases: #RULE 1: no repeated aliases
                continue
            if cfg.db.identity_alias.get(alias) is not None: #RULE 2: no repeated aliases
                continue
        ...
        for alias, pubkey, sig in cached_alias_updates:
            if alias in seen_aliases: #RULE 1: no repeated aliases
                continue
            if cfg.db.identity_alias.get(alias) is None: #RULE 4: only registed aliases
                continue
            old_pubkey = cfg.db.identity_alias.get(alias)
            if not ms.verify_data_signature( #RULE5: update sig must be valid
        ...
        for alias, nym, sig in self.cached_nym_updates:
            if alias in seen_aliases: #RULE 1: no repeated aliases
                continue
            if cfg.db.identity_alias.get(alias) is None: #RULE 6: only registed aliases
                continue
            pubkey = cfg.db.identity_alias.get(alias)
            if cfg.db.identity_nym.get(nym) is not None: #RULE 7: no repeated nyms
                continue
            nym_str = bytes(nym).decode("utf-8")
            pattern = r"^(?=.*[a-zA-Z])[a-zA-Z0-9_]{1," + str(pm.NYM_MAX_LENGTH) + r"}$"
            if not re.match(pattern, nym_str): #RULE 8: nym must match regex
                continue
            if not ms.verify_data_signature(pubkey, sig, bytes(alias), bytes(nym)): #RULE 9: nym sig must be valid
                continue
            seen_aliases.add(alias)
