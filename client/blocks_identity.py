...
import params as pm
from client.db_module import db_process_i_block, db_revert_i_block
...
def process_block(
    ...
    # SANITY CHECKS
    # Confirm identity block index matches client block index
...
def load_block(serialized_block: bytes) -> None:
    ...
    try:
        # check if block is valid and builds on chain tip
        ...
        if check_block_validity(new_block) and check_block_new(
            ...
            # resync chain from previous index
            index -= 1
            if index < 0:
                return
            # clear cached blocks
...
def receive_new_block(serialized_block: bytes) -> None:
    ...
    stores block in cahce db to be processed at a later time
    """
    offset = 0
...
def initiate_chain_sync(index: Index, init: bool = True) -> None:
    ...
    Query(msg.REQUEST_CHAIN_SYNC_I, check_chain_sync, block_hash, max_retries=10)
...
def sync_next_block(index: Index, init: bool = False) -> None:
    ...
    if cfg.show_sync:
        ...
        cfg.show_sync_vals()
    Query(msg.REQUEST_BLOCK_I, check_next_block, bytes(index), max_retries=10)
...
def check_next_block(serialized_block: bytes, b_index: bytes) -> None:
    ...
    index = Index(b_index)
    if (
        serialized_block == b"0"
    ):  # The chain tip has been reached #TODO come back to this
...
def revert_block(re_cache=False) -> None:
    ...
    Revert stale checkpointed semaphore blocks
    """
