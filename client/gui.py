...
def decompose(archive:bytes,epoch:Index) -> List[Broadcast]:
    ...
    while i < len(archive):
        # Extract bc_len which is now 2 bytes long
        bc_len = int.from_bytes(archive[i:i+2], 'big')
        i += 2  # increment by 2 since bc_len is 2 bytes
        ...
        try:
            if message[0] == '!':
                ...
                try:
                    ...
                    print("hex_parent", parent)
                    parent = BCPointer(bytes.fromhex(parent))
                    print("bytes_parent", parent)
