...
import node as nd
import time
from client.db_module import db_process_s_block, db_revert_s_block
...
def process_block(new_block: sb.SemaphoreBlock, serialized_block, index: Index) -> None:
    ...
    for i in new_block.replies_body:
        ...
        if cfg.show_messages:
            ...
            print(
                f"received: {nym.decode('utf-8')} -> {other_nym.decode('utf-8')}: {message.decode('utf-8')}"
            )
...
def initiate_chain_sync(index: Index, init: bool = True) -> None:
    ...
    Query(msg.REQUEST_CHAIN_SYNC_S, check_chain_sync, block_hash, max_retries=10)
...
def refine_chain_sync() -> None:
    ...
    Query(msg.REQUEST_CHAIN_SYNC_S, check_common_index, block_hash, max_retries=10)
...
def sync_next_block(index: Index, init: bool = False) -> None:
    ...
    Query(msg.REQUEST_BLOCK_S, check_next_block, bytes(index), max_retries=10)
